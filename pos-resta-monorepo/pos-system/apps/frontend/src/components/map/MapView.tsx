...
export const MapView: React.FC<MapViewProps> = ({ 
  ...
  const { data: ordersWithGeo } = useQuery({
    queryKey: ['orders-geo'],
  ...
  const { data: driverLocations, error: driverLocationsError } = useQuery({
    queryKey: ['driverLocations'],
    queryFn: employeesApi.getDriverLocations,
  ...
  useEffect(() => {
    if (!mapInstanceRef.current || !ordersWithGeo?.data) return;
    ...
    // Add driver markers
    if (driverLocations && 'data' in driverLocations && driverLocations.data) {
      console.log('📍 Adding driver markers:', driverLocations.data.length);
      driverLocations.data.forEach((driver: any) => {
        if (driver.latitude && driver.longitude && driver.isActive) {
          console.log('📍 Adding driver marker:', driver.driverName, 'at', driver.latitude, driver.longitude);
          const driverMarker = L.marker([driver.latitude, driver.longitude], {
            title: `Kierowca: ${driver.driverName}`,
          }).addTo(map);
          ...
          const driverPopupContent = `
            <div style="min-width: 200px; font-family: Arial, sans-serif;">
              ...
              <p style="margin: 4px 0; color: #666; font-size: 14px;"><strong>Ostatnia aktualizacja:</strong> ${new Date(driver.timestamp).toLocaleTimeString()}</p>
            </div>
          `;
          ...
          console.log('📍 Skipping driver marker (inactive or missing location):', driver.driverName, 'active:', driver.isActive, 'lat:', driver.latitude, 'lng:', driver.longitude);
        }
      });
  ...
  }, [ordersWithGeo, selectedOrder, filterType, driverLocations]);
