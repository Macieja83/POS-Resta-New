...
export const createOrder = async (orderData: CreateOrderRequest): Promise<Order> => {
  ...
  try {
    ...
    // Validate the response data with Zod schema (with safeParse to handle errors gracefully)
    const validationResult = OrderSchema.safeParse(posResponse.data);
    if (!validationResult.success) {
      console.error('Order validation errors:', validationResult.error.errors);
      // Return the order anyway if validation fails (for now)
      // The backend data structure might differ slightly
      return posResponse.data as Order;
    }
    console.log('Created order:', validationResult.data);
