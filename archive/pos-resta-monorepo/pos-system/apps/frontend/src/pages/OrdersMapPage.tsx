...
import { OrderType, PaymentMethod } from '../types/shared';
...
export const OrdersMapPage: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    refetchOnMount: false, // Use cache on mount for faster loading
    gcTime: 10 * 60 * 1000, // Keep in cache for 10 minutes
    retry: 1, // Retry once on failure (faster)
    retryDelay: 200, // Wait 200ms before retry (faster)
  ...
  React.useEffect(() => {
    if (data?.data) {
      ...
      if (selectedType === 'in_progress') {
        filteredOrders = data.data.filter((order: any) => 
  ...
  useEffect(() => {
    const handleFilterChange = (event: CustomEvent) => {
      setSelectedType(event.detail.type);
    };
              ...
              <>
                ...
                >
                  ...
                  <div className="order-col client">
                    {(order.promisedTime || order.status === 'OPEN' || order.status === 'IN_PROGRESS') && (
                      <div>
                        <CountdownTimer 
